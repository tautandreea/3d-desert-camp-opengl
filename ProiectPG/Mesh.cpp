...
namespace gps {
	...
	/* Mesh Constructor */
	Mesh::Mesh(std::vector<Vertex> vertices, std::vector<GLuint> indices, std::vector<Texture> textures, glm::vec3 baseColor, bool hasDiffuseTexture) {
	...
	void Mesh::Draw(gps::Shader shader)	{
		...
		glUniform3fv(glGetUniformLocation(shader.shaderProgram, "baseColor"), 1, glm::value_ptr(this->baseColor));
		glUniform1i(glGetUniformLocation(shader.shaderProgram, "useTexture"), this->hasDiffuseTexture);
